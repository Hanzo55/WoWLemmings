<cfcomponent output="false">
	...
	<!--- <cffunction name="init" returntype="com.hanzo.cf.service.KathuneXMLService" access="public" output="false">
	
		<cfset variables.kathune = createObject('component','com.hanzo.cf.Kathune.Kathune').init('/config.xml') />
		
		<cfreturn this />
	</cffunction> --->
	
	<cffunction name="GetRecruits" returntype="xml" access="remote" output="false">

		<cfset var recruitQuery = 0 />
		<cfset var recruitXML = 0 />

		<cfset recruitQuery = application.kathune.GetRecruits() />
		
		<cfset recruitXML = QueryToXML(recruitQuery) />
